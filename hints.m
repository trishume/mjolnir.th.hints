...
#define get_hint_arg(L, idx) *((HintWindow**)luaL_checkudata(L, idx, "mjolnir.th.hints.hint"))
...
+ (void)initCache {
  if(!createdCache) {
    ...
    hintBackgroundColor = [[NSColor colorWithWhite:0.0 alpha:0.65] retain];
    hintFontColor = [NSColor whiteColor];
    hintFont = [[NSFont systemFontOfSize:25.0] retain];
    hintIconAlpha = 0.95;
...
- (CGSize)updateFrameSize {
  CGRect newFrame = NSMakeRect(self.frame.origin.x, self.frame.origin.y, textSize.width + 100, hintHeight);
  self.frame = newFrame;
...
- (HintWindow*)initWithPoint:(CGPoint)pt text:(NSString*)txt
  ...
  if (self) {
    ...
    self.contentSize = [label updateFrameSize];
...
void new_hint(lua_State* L, HintWindow* screen) {
  HintWindow** hintptr = lua_newuserdata(L, sizeof(HintWindow**));
  *hintptr = [screen retain];

  luaL_getmetatable(L, "mjolnir.th.hints.hint");
...
int luaopen_mjolnir_th_hints_internal(lua_State* L) {
  ...
  if (luaL_newmetatable(L, "mjolnir.th.hints.hint")) {
    lua_pushvalue(L, -2);
